...
<script>let run=null;const $=id=>document.getElementById(id),set=(t)=>$('status').textContent=t;$('date').value=new Date().toISOString().slice(0,10);function show(a){$('article').textContent=a?.markdown||''}async function call(url,body){const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok)throw Error(d.error||'请求失败');return d}$('prepare').onclick=async()=>{try{set('正在生成…');run=await call('/api/runs',{date:$('date').value,retry:true});show(run.article);$('publish').disabled=run.state!=='ready';set(run.state==='ready'?'文章已生成，尚未发布':(run.error?'失败：'+run.error:run.state))}catch(e){set('失败：'+e.message)}};$('publish').onclick=async()=>{try{set('正在创建草稿…');run=await call('/api/runs/'+run.id+'/publish',{date:$('date').value});set('草稿已创建：'+run.media_id);$('publish').disabled=true}catch(e){set('失败：'+e.message)}};</script></html>""", mimetype="text/html")
