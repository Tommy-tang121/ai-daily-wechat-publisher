...
- 当天全部资讯只发起一次模型请求；该请求同时返回今日观察、每条改写和小编短评。
- 当 `LLM_BASE_URL` 为 `https://api.deepseek.com` 时，请求会关闭思考模式并要求 JSON 对象返回；本地严格校验仍是最终防线。
- 每条保留原始来源链接；模型返回链接不会被采纳。
- 模型 JSON 无效、缺少今日观察或小编短评、或返回条目数与抓取数不一致时，整条运行失败，绝不静默遗漏或发布原文。
...
- 根目录 `.env` 仅在本地读取；`app/.env` 仍可兼容覆盖同名配置。两者均不提交，且密钥不进入日志。
