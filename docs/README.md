...
- 本地只暂存处理中的运行、`ready` 文章和创建草稿失败后可重试的内容。微信草稿创建成功后，会立即清除本地文章、事件、封面和运行记录；不保留已发布历史或草稿回执。
- 手动选择任意日期点击“抓取”会开始一次新的生成；同一天已完成或失败的旧内容不会被复用。
- 处理超过 30 分钟仍无进展的排队、抓取、改写或发布任务，才允许定时重试接管；正常运行不会重复执行。
- 定时任务按运行日期生成标题。文章已生成但创建草稿失败时，会保留文章并仅重试创建草稿，不会重新调用模型。
