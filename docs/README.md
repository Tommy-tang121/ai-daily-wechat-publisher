...
- 抓取和改写每次上报有效进度都会刷新运行心跳；只有连续超过 30 分钟没有进度的排队、抓取或改写任务，才允许定时重试接管。`publishing` 即使过期也绝不接管或重发，而是在下次状态检查时转为“发布结果待确认”。若定时任务发现一个仍健康、自己并不拥有的排队或处理中任务，会以失败退出，让 Windows 任务按既有规则重试，而不是静默漏掉当天日报。清理开始后，旧线程再上报进度会被忽略，页面也不会显示旧事件。
